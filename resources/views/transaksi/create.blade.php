            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('transaksi.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jenis_gigi_id" id="jenis_gigi_id"
                                class="w-full mt-1 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                            <option value="">-- Pilih Jenis Gigi --</option>
                            @foreach ($jenisGigis as $gigi)
                                <option value="{{ $gigi->id }}" {{ old('jenis_gigi_id') == $gigi->id ? 'selected' : '' }}>
                                    {{ $gigi->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="ongkir" id="ongkir" min="0" value="{{ old('ongkir', 0) }}"
                               class="w-full mt-1 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('transaksi.index') }}"
                           class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-md mr-2">
                            Batal
                        </a>
                        <button type="submit"
